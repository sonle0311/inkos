...
export function analyzeHookHealth(params: {
  ...
  if (activeHooks.length > maxActiveHooks) {
    issues.push(warning(
      params.language,
      params.language === "zh" ? `当前有 ${activeHooks.length} 个活跃伏笔，已经高于建议上限 ${maxActiveHooks} 个。` : `There are ${activeHooks.length} active hooks, above the recommended cap of ${maxActiveHooks}.`,
      params.language === "zh" ? "优先推进、回收或延后已有伏笔，再继续开新伏笔。" : "Prefer advancing, resolving, or deferring existing debt before opening more hooks.",
    ));
  }
  ...
  if (unresolvedPressure.length > 0) {
    issues.push(warning(
      ...
      params.language === "zh" ? "先让一个已进入压力区的伏笔发生真实推进、回收或明确延后，再继续扩展同类债务。" : "Move one pressured hook with a real payoff, escalation, or explicit defer before opening adjacent debt.",
    ));
  } else {
    ...
    if (
      ...
      issues.push(warning(
        params.language,
        params.language === "zh" ? `已经连续 ${params.chapterNumber - latestRealAdvance} 章没有真实伏笔推进。` : `No real hook advancement has landed for ${params.chapterNumber - latestRealAdvance} chapters.`,
        params.language === "zh" ? "下一章优先让一个旧伏笔发生真实推进，而不是继续平行重述。" : "Schedule one old hook for real movement instead of opening parallel restatements.",
      ));
    }
  }
  ...
  if (params.delta) {
    ...
    if (newHookIds.length >= newHookBurstThreshold && params.delta.hookOps.resolve.length === 0) {
      issues.push(warning(
        params.language,
        params.language === "zh" ? `本章新开了 ${newHookIds.length} 个伏笔，但没有回收任何旧债。` : `Opened ${newHookIds.length} new hooks without resolving any older debt.`,
        params.language === "zh" ? "控制伏笔膨胀，新开伏笔时尽量配套回收旧伏笔。" : "Keep the hook table from ballooning by pairing new openings with old payoffs.",
      ));
    }
  }
...
function buildPressureDescription(params: {
  ...
  const summarized = params.entries
    ...
    .map(({ hook, lifecycle }) => {
      ...
      return params.language === "zh" ? `${hook.hookId}（${timing}，${pressure}）` : `${hook.hookId} (${timing}, ${pressure})`;
    });
  const suffix = params.entries.length > summarized.length
    ? params.language === "zh" ? `，另有 ${params.entries.length - summarized.length} 条` : `, +${params.entries.length - summarized.length} more`
    : "";

  if (params.language !== "zh") { return params.mentionsCurrentChapter
...
function localizePressureLabel(
  ...
  if (lifecycle.readyToResolve) {
    return language === "zh" ? "可回收" : "ready to pay off";
  }
  return language === "zh" ? "陈旧" : "stale";
}
...
function warning(
  ...
  return {
    severity: "warning",
    category: language === "zh" ? "伏笔债务" : "Hook Debt",
    description,
    suggestion,
  };
}
