...
export function sanitizeNarrativeControlText(
  ...
  result = result.replace(HOOK_ID_PATTERN, language === "zh" ? "这条线索" : "this thread");
  result = result.replace(HOOK_SLUG_PATTERN, language === "zh" ? "这条线索" : "this thread");
  for (const pattern of CHAPTER_REF_PATTERNS) {
    result = result.replace(pattern, language === "zh" ? "此前" : "an earlier scene");
  }
...
export function buildNarrativeIntentBrief(
  ...
  const sections = [
    { heading: "## Goal", label: language === "zh" ? "目标" : "Goal" },
    { heading: "## Outline Node", label: language === "zh" ? "当前节点" : "Outline Node" },
    { heading: "## Must Keep", label: language === "zh" ? "保留" : "Keep" },
    { heading: "## Must Avoid", label: language === "zh" ? "避免" : "Avoid" },
    { heading: "## Style Emphasis", label: language === "zh" ? "风格" : "Style" },
    { heading: "## Structured Directives", label: language === "zh" ? "指令" : "Directives" },
  ] as const;
...
export function renderNarrativeSelectedContext(
  ...
  const heading = language === "zh" ? "证据" : "Evidence";
  const reasonLabel = language === "zh" ? "原因" : "reason";
  const detailLabel = language === "zh" ? "细节" : "detail";
