  ...
  try {
    ...
    const failedSession = appendInteractionEvent({
      ...
      currentExecution: {
        ...
        stageLabel: localizedRequest.language === "zh" ? `执行失败：${localizedRequest.intent}` : `failed ${localizedRequest.intent}`,
      },
    }, {
      kind: "task.failed",
      timestamp: Date.now(),
...
async function detectProjectInteractionLanguage(projectRoot: string): Promise<"zh" | "en" | "vi" | undefined> {
  try {
    ...
    return parsed.language === "zh" ? parsed.language === "zh" ? "zh" : undefined : "en";
  } catch {
    return undefined;
  }
}
