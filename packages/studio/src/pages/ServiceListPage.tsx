  ...
  useEffect(() => {
    ...
    void fetchJson<CoverConfigPayload>("/cover/config")
      ...
      .catch((error) => {
        if (cancelled) return;
        setStatus("error");
        setMessage(error instanceof Error ? error.message : tr("读取封面配置失败", "Failed to load cover config", "Không tải được cấu hình bìa"));
      });
  ...
  return (
    <section className="rounded-xl border border-border/50 bg-card/50 p-4 space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="space-y-1.5">
          <span className="block text-xs font-medium text-muted-foreground/70">{tr("封面模型", "Cover model", "Mô hình tạo bìa")}</span>
          <select
            value={model}
      ...
      <label className="space-y-1.5">
        ...
        <span className="block text-[11px] leading-5 text-muted-foreground/55">
          {tr(
            ...
            "Để trống để dùng địa chỉ mặc định của nhà cung cấp; địa chỉ tùy chỉnh sẽ trở thành gốc API tạo bìa.",
          )}
        </span>
      </label>
