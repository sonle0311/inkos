  ...
  return (
    <div data-testid="emotion-arc" className="border border-border rounded p-3">
      ...
      {displayArcs.length === 0 ? (
        <div className={`text-sm ${c.muted}`}>{tr("暂无可分析路径", "No paths to analyze", "Chưa có đường đi để phân tích")}</div>
      ) : (
        <>
          <svg
            width="100%"
          ...
          {(overLimit || arcs.truncated) && (
            <div className={`text-xs mt-1 ${c.muted}`}>
              {overLimit && tr(`仅显示前 ${MAX_ARC_DISPLAY} 条路径`, `Showing first ${MAX_ARC_DISPLAY} paths only`, `Chỉ hiển thị ${MAX_ARC_DISPLAY} đường đi đầu`)}
              {arcs.truncated && tr("（路径总数已超过枚举上限）", " (total paths exceed the enumeration limit)", " (tổng số đường đi vượt giới hạn liệt kê)")}
            </div>
          )}
        </>
      )}
...
function PathDistributionPanel({
  ...
  return (
    <div data-testid="path-distribution" className="border border-border rounded p-3">
      <div className={`text-sm font-medium mb-2 ${c.muted}`}>{tr("路径分布", "Path distribution", "Phân bố đường đi")}</div>

      {distribution.truncated && (
        <div className={`text-xs mb-2 ${c.muted}`}>
          {tr(`路径过多，仅统计前 ${distribution.total} 条`, `Too many paths; only the first ${distribution.total} are counted`, `Quá nhiều đường đi; chỉ thống kê ${distribution.total} đường đầu tiên`)}
        </div>
      )}

      {endingEntries.length === 0 ? (
        <div className={`text-sm ${c.muted}`}>{tr("暂无路径数据", "No path data", "Chưa có dữ liệu đường đi")}</div>
      ) : (
      ...
      {histEntries.length > 0 && (
        <div>
          <div className={`text-xs font-medium mb-2 ${c.muted}`}>{tr("路径长度分布", "Path length distribution", "Phân bố độ dài đường đi")}</div>
              ...
              return (
                ...
                >
                  <div
                    ...
                    title={tr(`长度 ${len}: ${count} 条`, `Length ${len}: ${count} paths`, `Độ dài ${len}: ${count} đường`)}
                  />
